...
if target_url:
    ...
    if instance_of(df_master, pd.DataFrame) and not df_master.empty:
        keyword = st.text_input("Filter by Keyword", "")
        start_dt = from_date.combine(from_dt, datetime.min.time()).replace(tzinfo=timezone.utc)
        ...
        safe_url = sanitize_for_filename(target_url)
    ...
    elif instance_of(df_master, str):
